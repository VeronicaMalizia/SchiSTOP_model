...
setting <- "Moderate_Complete" #"Moderate_setting_k=015_Age-int"
endem <- "Moderate"
...
#Computing runs that get eliminated after MDA:
eliminated <- res %>%
  filter(time==3600) %>%
  group_by(Immunity, Snails, DDF) %>%
  summarise(dead_seed = length(which(eggs_prev_SAC==0))*100/seeds)
...
kable(caption = "Eliminated runs after 10 ys MDA",
      eliminated %>%
          mutate(dead_seed = paste(dead_seed, "%", sep = " "))
      )
}
```
...
Fig <- ggplot(data=data_avg, aes(x=time/12)) +
  geom_line(data = res, aes(y=eggs_prev_SAC*100, group = seed), colour = "grey", alpha = 0.05) +
...
if(endem=="High" | endem=="Moderate"){
  data_all <- data_all[- which(data_all$Immunity=="Absent" & data_all$Snails=="Absent" & data_all$DDF=="Absent"),]
}
